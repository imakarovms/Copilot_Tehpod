...
|---|---|---|
| Эмбеддинги | paraphrase-multilingual-MiniLM-L12-v2 | Векторизация текста (384d) |
| Векторная БД | ChromaDB | Хранение и поиск эмбеддингов |
| Лексический поиск | BM25 + pymorphy3 | Поиск по ключевым словам с лемматизацией |
| Fusion | Reciprocal Rank Fusion | Объединение результатов поиска |
| LLM | Qwen2.5-7B-Instruct (GGUF Q4_K_M) | Генерация ответов |
| LLM Runtime | llama-cpp-python | Локальный инференс на GPU |
| API | FastAPI + Uvicorn | REST API |
| CI/CD | GitHub Actions | Автоматический линтинг и тесты |
