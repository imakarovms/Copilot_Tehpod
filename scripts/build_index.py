...
def main():
    ...
    if not index_path.exists():
        ...
        logger.info("Создайте его вручную для теста или запустите scripts/generate_dataset.py")
        sys.exit(1)
    ...
    if db_count != len(tickets):
        logger.error("ПРОВАЛ: в базе %d тикетов, а ожидалось %d", db_count, len(tickets))
        sys.exit(1)
